...
#### Running multiple sessions? Each one logs in on its own

Each `REMOTE_SESSION_NAME` gets its own isolated credentials folder under
`CONFIG_BASE_PATH` — even two sessions sharing the same `CONFIG_BASE_PATH`
each prompt for their own login. There's no way to share one login across
sessions; attach to each new session once and complete the authentication
flow, same as the first one.
