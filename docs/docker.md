...
**On `SHARED_CREDENTIALS_PATH` specifically:** by default, every `REMOTE_SESSION_NAME` gets its own isolated credentials — even sessions sharing the same `CONFIG_BASE_PATH` still get separate subfolders (`CONFIG_BASE_PATH/<session>`), so each one prompts for its own login. `SHARED_CREDENTIALS_PATH` is the opt-in way around that: point every session's `.env` at the *same* host directory — it doesn't need to exist beforehand, Docker creates it empty on first use, unlike a single-file mount — and the first session to log in seeds a credentials file inside it; every session started afterward loads from it instead of prompting. It's mounted to a dedicated directory (`/home/node/.claude-shared-credentials`), not straight onto the session's own `.credentials.json`, because Compose can't express "mount this here, but only if the variable is set, otherwise leave whatever's already there alone" — `entrypoint.sh` does the actual copy between the two paths at startup, and also checks the mounted directory is writable by the container's user (warns non-fatally and skips the sync if not — this can happen if the host path didn't exist yet and Docker auto-created it root-owned). This is a one-time sync per container start, not a live share: a token that rotates inside an already-running session doesn't propagate to the shared directory or to other running containers until they restart.
