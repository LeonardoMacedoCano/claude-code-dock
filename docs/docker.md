...
**Important note:** The config volume mounts to `/home/node/.claude` (not `/root/.claude`), because the actual Claude Code process runs as the `node` user (UID/GID 1000 by default, non-root — set `PUID`/`PGID` in `.env` if your host user differs). It must be writable by that UID on the host — if `CONFIG_BASE_PATH` is unset it silently falls back to `./configs`, and Docker will auto-create that as root-owned. The `claude-code-dock-init` service (see `docker-compose.yml`) chowns it to `PUID`/`PGID` automatically on every `docker compose up`, before the main container ever starts, so this normally self-heals; if it somehow doesn't, the entrypoint still rejects an unwritable directory at startup (see [Container restart loop](#container-restart-loop) below) instead of crash-looping silently.
...
2. The config or workspace directory is not writable by UID 1000 (`node`) — usually because `CONFIG_BASE_PATH`/`WORKSPACE_PATH` is unset or misspelled. The `claude-code-dock-init` service in `docker-compose.yml` already chowns both automatically on every `docker compose up` (see `docs/security.md#known-exception--the-permission-fixing-init-container`), so if this still fires, check `docker logs <container_name>-init` first — the filesystem likely refused the `chown` (NFS root-squash, some rootless setups)
