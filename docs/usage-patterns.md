...
> **Note:** this page documents patterns achievable **today**, with the
> current variables. It complements, and does not depend on, the proposed
> directory reorganization in
> [`docs/multi-instance-architecture-proposal.md`](multi-instance-architecture-proposal.md)
> (still a draft under review — nothing in that document is implemented
> yet). Everything below works with claude-code-dock as it ships right now.
