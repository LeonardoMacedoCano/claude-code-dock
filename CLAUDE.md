...
| Share one Claude Code login across sessions instead of one per `REMOTE_SESSION_NAME` | `SHARED_CREDENTIALS_PATH` (+ optional `SHARED_CREDENTIALS_MODE`) | Silent no-op when `SHARED_CREDENTIALS_PATH` unset (mounted to `/dev/null`, gated on `-d`); warns (non-fatal) if the host directory isn't writable by the container's user. `SHARED_CREDENTIALS_MODE` itself is a no-op with no `SHARED_CREDENTIALS_PATH` set; defaults to `live` (the original symlink+poller mechanism, unchanged) when unset — see the `docker/entrypoint.sh` and `scripts/promote-credentials.sh` entries below for `seed` |
...
- **`SHARED_CREDENTIALS_PATH` (optional):** a dedicated *directory* mount,
  `SHARED_CREDENTIALS_PATH:/home/node/.claude-shared-credentials`, same
  ...
  own — `entrypoint.sh` copies `~/.claude/.credentials.json` to/from a fixed
  filename inside this directory once at startup (see [File
  Responsibilities](#file-responsibilities) below), never straight onto
  `~/.claude/.credentials.json` itself, since Compose has no leak-free way to
  make a volume line conditional (same constraint as `CLAUDE_SOURCE_PATH`'s
  build context, below) — mounting straight onto that path would shadow
  every session's real credentials with the `/dev/null` fallback for anyone
  who never opted in. A directory rather than a single file specifically so
  a host path that doesn't exist yet is just created empty — a single-file
  mount would instead turn a missing host file into an unusable directory at
  the target, forcing a manual `touch` before the very first use.
...
| `INSTANCE_CONFIG_PATH` | `` | Optional, additive: an explicit config directory path that wins over `CONFIG_BASE_PATH`/`REMOTE_SESSION_NAME` when set, resolved identically by `docker-compose.yml`'s volume interpolation and by every host script via `scripts/lib/config-path.sh`. Unset, nothing changes for anyone |
...
| `SHARED_CREDENTIALS_PATH` | `` | HOST path to a directory used to share one Claude Code login across sessions instead of one per `REMOTE_SESSION_NAME`. Doesn't need to exist beforehand — created empty on first use. Unset falls back to `/dev/null`, same idiom as `GITHUB_TOKEN_FILE`/`GLOBAL_CONFIG_PATH` — a no-op for anyone who doesn't set it |
| `SHARED_CREDENTIALS_MODE` | `live` | `live` (original symlink+poller mechanism, unchanged) or `seed` (one-time copy at boot, no live link, no poller — promotion the other direction is the separate, explicit `scripts/promote-credentials.sh`). A no-op with no `SHARED_CREDENTIALS_PATH` set |
| `TZ` | `UTC` | Timezone |
...
- `container_name: ${CONTAINER_NAME:-claude-code-dock}`: name driven by `.env`; scripts depend on a stable, known name. The unset default is the flat literal `claude-code-dock`, **not** derived from `REMOTE_SESSION_NAME` — `install.sh`/`new-session.sh` compensate by auto-writing `CONTAINER_NAME=claude-code-dock-<session>` into the `.env` they generate. See [docs/architecture.md — Design Decisions](docs/architecture.md#design-decisions) for why deriving the default from `REMOTE_SESSION_NAME` instead was rejected.
...
- `- ${SHARED_CREDENTIALS_PATH:-/dev/null}:/home/node/.claude-shared-credentials` in `volumes:`: same optional-file-mount idiom, but **not** `:ro` — `entrypoint.sh` writes to this path too (`~/.claude/.credentials.json` is symlinked here, so every login/token write lands directly on this mount) — and a directory, not a single file, so an operator setting `SHARED_CREDENTIALS_PATH` to a not-yet-existing host path gets it auto-created empty instead of the single-file idiom's usual "missing host file becomes an unusable directory at the mount target" gotcha. Deliberately targets a dedicated, otherwise-unused path rather than `/home/node/.claude/.credentials.json` directly: the `/dev/null` fallback would then shadow every session's real credentials file for anyone who never set `SHARED_CREDENTIALS_PATH`, the same class of problem the `CLAUDE_SOURCE_PATH` interpolation rule below exists to avoid.
...
- `- ${INSTANCE_CONFIG_PATH:-${CONFIG_BASE_PATH:-./configs}/${REMOTE_SESSION_NAME:-default}}:/home/node/.claude` in `volumes:`: `INSTANCE_CONFIG_PATH`, when set, is used directly as the mount source, winning over the `CONFIG_BASE_PATH`/`REMOTE_SESSION_NAME` join — the same nested-interpolation idiom already used by `build.context` above (`${CLAUDE_SOURCE_PATH:-...${CLAUDE_DOCK_VERSION:-main}}`), not a new pattern. Purely additive: unset, this resolves identically to the join Compose has always used. `- INSTANCE_CONFIG_PATH=${INSTANCE_CONFIG_PATH:-}` in `environment:` is informational only (mirrors `CONFIG_BASE_PATH`'s own passthrough) — it's read inside the container only to build an accurate `chown` hint in `entrypoint.sh`'s `fatal()` message, never to re-derive the mount, which is already fixed by the time the container starts.
...
- Do not point the `SHARED_CREDENTIALS_PATH` volume line straight at `/home/node/.claude/.credentials.json`, and do not make it a single-file mount — see the bullet above; the `/dev/null` fallback would silently break default (non-shared) credential persistence for every operator who hasn't set the variable, and a single-file mount reintroduces the "missing host file becomes an unusable directory" gotcha this design specifically avoids
...
10. `SHARED_CREDENTIALS_PATH` link (right after the `~/.claude.json` symlink block, before `HAS_CREDENTIALS` is read): this whole block is gated first on `SHARED_CREDENTIALS_MODE` (default `live`, unset resolves to `live`, the mechanism described below and completely unchanged by the variable's introduction). `SHARED_CREDENTIALS_MODE=seed` branches to a separate, much shorter code path instead — see the dedicated bullet below the poller description — that never creates a symlink or a poller at all, so everything below through the poller bullet describes `live` mode only. In `live` mode: symlinks `~/.claude/.credentials.json` (`SESSION_CREDS`, this session's own, always inside the per-session mount) to a fixed filename (`.credentials.json`) inside `~/.claude-shared-credentials/` (`SHARED_CREDS_FILE`, the dedicated directory mount fed by `SHARED_CREDENTIALS_PATH`, see the `docker-compose.yml` section above) — the same idiom as the `CLAUDE_JSON_LINK`/`CLAUDE_JSON_REAL` symlink just above it, not a copy. Gated on `[ -d "${SHARED_CREDS_DIR}" ]` — true only when the operator actually set `SHARED_CREDENTIALS_PATH` to a real host directory, since the unset `/dev/null` fallback is a character special file, not a directory, so `-d` is false and the whole block is a no-op.
    - **Unwritable directory:** warns non-fatally and skips the link — this session still logs in on its own, it just doesn't join the shared pool until the operator fixes ownership. If `SESSION_CREDS` is *already* a symlink from a prior successful boot (the mount just turned unwritable, e.g. a NAS remount gone read-only), it also attempts a best-effort `cp` through the existing link to materialize a real local file in its place, so this session keeps working standalone this run instead of being stranded behind a link `claude` can no longer write through — it rejoins the shared pool automatically once writability returns and this session restarts.
    - **Writable directory:** if `SESSION_CREDS` is still a *regular* file (a login predating `SHARED_CREDENTIALS_PATH` being set, or a pre-symlink claude-code-dock version), its content — if non-empty — is copied into `SHARED_CREDS_FILE` before the file itself is removed, and a login this session already performed always wins (a `log_warn` fires first if `SHARED_CREDS_FILE` already held *different* non-empty content, since this overwrites it — non-fatal, but no longer silent). `SESSION_CREDS` is then `ln -sf`'d to `SHARED_CREDS_FILE` unconditionally, and a log line always fires either way (`skips first login` when the pool already had a login, `pool currently empty` otherwise) — there is no silent path through this block anymore.
    - **The symlink alone does not survive a login or token refresh — confirmed by incident.** `claude` does not write `.credentials.json` in place (`O_TRUNC`); it writes a new file and `rename()`s it over the old path, which detaches whatever was at that path — including this symlink — and leaves a plain regular file behind, in `SESSION_CREDS`'s own directory, with `SHARED_CREDS_FILE` untouched. A real container hit exactly this: the symlink was established correctly at boot, but the very first login left the shared pool completely empty and the login sitting only in the session's own config dir. Neither a privileged bind-mount (rename onto an actual mountpoint fails with `EBUSY` — worse, not better) nor `CLAUDE_CONFIG_DIR` (relocates the *entire* `~/.claude`, not just this one file) fixes this from outside the container. The fix is a plain-bash poller (no new package), backgrounded (`&`; `disown`) right after the initial link is established: every 5s it checks whether `SESSION_CREDS` turned back into a real file, and if so immediately repeats the promote-then-link step and logs it (`log_warn` that a live write was detected, then `log_info` once re-linked, or `log_warn` if `SHARED_CREDENTIALS_PATH` itself couldn't be written to). This survives the final `exec tmux ...` below — `exec` replaces this script's process image but keeps its PID and already-forked children, only exiting would detach them — and needs no explicit shutdown handling, since it dies with the container's PID namespace. Net effect: a login/refresh leaves the file outside `SHARED_CREDENTIALS_PATH` for at most a few seconds, never until the next restart.
    - **Backup interaction:** see `scripts/backup.sh`'s entry above — a symlink here means `scripts/backup.sh` has to dereference `.credentials.json` specifically before taring `CONFIG_DIR`, or the archive would silently contain no actual login.
    - **`SHARED_CREDENTIALS_MODE=seed`:** no symlink, no poller, no automatic promotion in either direction. At boot: if `SESSION_CREDS` already exists (a real file — this session already has its own login), the shared pool is not consulted at all. Otherwise, if `SHARED_CREDS_FILE` is non-empty, it's `cp`'d (not linked) into `SESSION_CREDS` — a one-time provision; from that point on `SESSION_CREDS` is a completely ordinary file in this session's own directory, so `claude`'s `rename()`-based writes (the whole reason the `live`-mode poller exists) need no special handling at all. If neither has anything, login proceeds normally. Getting a session's login *into* the shared pool is `scripts/promote-credentials.sh` (see its own entry below) — a separate, operator-run, host-side script, not anything `entrypoint.sh` does automatically. This mode exists specifically to avoid the no-locking multi-writer risk `live` mode has when several sessions promote near-simultaneously (each one's `SESSION_CREDS` being a *live* link means every already-linked session immediately and silently starts resolving to whichever login won, not just new sessions — see `tests/entrypoint_shared_credentials_race.bats`, which characterizes this for `live` mode specifically).
...
**Why pull, not build, by default:** the published image is rebuilt in CI on every push to `main`, and on demand via a manual `workflow_dispatch` run, with a cache-busting build arg on the `npm install -g @anthropic-ai/claude-code` layer specifically, so neither path ever serves a stale version from the GitHub Actions layer cache — see `.github/workflows/docker-publish.yml`. Pulling that image is faster than rebuilding locally and gets the same freshness guarantee. `--no-cache` remains necessary for the local-build fallback path, for the same cache-staleness reason. There is deliberately no scheduled/cron trigger — a rebuild only ever happens because a commit landed on `main` or because someone explicitly ran the workflow, never on a timer with no action behind it.

**`:stable` tag (opt-in, manual only):** every push to `main` (and any manual `workflow_dispatch` run from `main`, since the `latest` tag rule keys off the branch, not the trigger type) moves `:latest` — by design, so it stays current with `claude-code-dock` changes without anyone having to remember to update it. That also means `:latest` can, in principle, pick up a `claude` CLI release that breaks something, with no staging step — to pull a fresh `@anthropic-ai/claude-code` release deliberately (not tied to a `claude-code-dock` commit), a maintainer runs `workflow_dispatch` manually. `docker-publish.yml`'s `workflow_dispatch` accepts a `promote_stable` boolean input (default `false`); when run with it checked, that specific build is additionally tagged and pushed as `:stable` (`type=raw,value=stable,enable=${{ github.event_name == 'workflow_dispatch' && inputs.promote_stable == true }}` in the `Extract metadata` step). `:stable` never moves on its own — only ever via that explicit manual action, after whoever runs it has actually tried the build. Users who want to avoid riding an untested `latest` bump set `CLAUDE_DOCK_TAG=stable` in `.env` instead of the default `latest`.
...
**What is backed up:**
- `CONFIG_BASE_PATH/REMOTE_SESSION_NAME` (or `INSTANCE_CONFIG_PATH` when set — Claude Code credentials) — always included
- `./workspaces/` (local workspace) — included if not empty
- External workspace — only with `--include-workspace`

**Destination (`OUTPUT_DIR`):** `./backups/<REMOTE_SESSION_NAME>/` once a session name is known — set once in `load_env()`, right after `BACKUP_PATTERN` is determined, gated on `OUTPUT_DIR_EXPLICIT` (set only by `--output`) so an operator-specified destination is never silently redirected into a subfolder. No `REMOTE_SESSION_NAME` (the `configs/default` fallback case) keeps the legacy flat `./backups/` layout, matching `BACKUP_PATTERN`'s own existing branch on the same condition. `restore.sh` is the counterpart that has to look in both places — see its own entry below.

**`SHARED_CREDENTIALS_PATH` sessions (`create_backup_archive()`):** when `CONFIG_DIR/.credentials.json` is a symlink (a session using `SHARED_CREDENTIALS_PATH`, see `docker/entrypoint.sh` development rule 10), `tar` would otherwise store the symlink itself — its target path as text, not the file it points to, since nothing in this script passes `-h`/`--dereference` — silently producing a backup with no actual login in it despite the "always included" claim above. `create_backup_archive()` detects this case and stages a copy of `CONFIG_DIR` in a temp dir with only `.credentials.json` dereferenced into a real file before taring that staged copy instead of `CONFIG_DIR` directly; every other symlink under `CONFIG_DIR` (global commands/skills from `GLOBAL_CONFIG_PATH`) is deliberately left as a symlink, since those are regenerated from their source on every container startup and hold no unique state worth dereferencing. `restore.sh` needs no changes for this — the restored archive has a plain `.credentials.json` file at the same path it always did, which `entrypoint.sh`'s promote step (rule 10) turns back into a link to `SHARED_CREDENTIALS_PATH` on that session's next boot.

**Resolving `CONFIG_BASE_PATH`/`REMOTE_SESSION_NAME`/`WORKSPACE_PATH`/`INSTANCE_CONFIG_PATH` (`load_env()`):** an already-exported process env var takes priority; `.env` on disk is only consulted for whatever isn't already set — same "`.env` is not the only valid source" principle as the GitHub-auth checks earlier in this file. Do not reintroduce an unconditional reset of these to `""` before reading `.env` — that was a real bug: it silently discarded an already-exported value and fell back to the wrong (usually empty) `./configs/default`, backing up nothing while looking like it succeeded. `CONFIG_DIR` itself is resolved via `scripts/lib/config-path.sh`'s `resolve_config_dir()` (shared with `restore.sh`/`status.sh`), not inline here — see that file's own entry below.
...
**Two-location backup search (`_backup_search_paths()`, `INSTANCE_BACKUP_DIR`):** `--list`, and the "no file specified" default resolution, both glob `INSTANCE_BACKUP_DIR` (the new per-instance subfolder — see `scripts/backup.sh` above) **and** `DEFAULT_BACKUP_DIR` (the legacy flat `./backups/`), combined via a single `ls -1t` call so results from both locations sort together by time — not two separate lists. This is what lets a backup taken before the per-instance subfolder existed stay listable/restorable with zero migration step. `INSTANCE_BACKUP_DIR` only differs from `DEFAULT_BACKUP_DIR` when `REMOTE_SESSION_NAME` is set; `_backup_search_paths()` skips the redundant second glob term when they're equal (an unnamed/default session), rather than passing the same path to `ls` twice and double-counting matches.

---

### `scripts/lib/config-path.sh`

**Responsibility:** Not a standalone script — sourced (`source "${SCRIPT_DIR}/lib/config-path.sh"`) by `backup.sh`/`restore.sh`/`status.sh`/`promote-credentials.sh`. Its one function, `resolve_config_dir()`, sets `CONFIG_DIR` (and normalizes `CONFIG_BASE_PATH`'s `./`-prefix in place) from either `INSTANCE_CONFIG_PATH` (wins when set) or `CONFIG_BASE_PATH`/`REMOTE_SESSION_NAME`, returning `1` when neither is usable so each caller can apply its own fallback path and messaging — those legitimately differ per script (`backup.sh` warns and falls back to `configs/default`; `restore.sh`/`status.sh` fall back silently), which is why this helper only owns the path-computation logic itself, not the full "what do I do when there's no config dir" decision.

**Test isolation:** any bats test that copies a script (`backup.sh`, `restore.sh`, etc.) into its own isolated `TMP_PROJECT/scripts/` directory must also `cp -r scripts/lib "$TMP_PROJECT/scripts/lib"` — the sourced script is resolved relative to `SCRIPT_DIR`, so a copy without its sibling `lib/` fails at the `source` line. `tests/backup_retention.bats`/`backup_restore.bats`/`instance_config_path.bats`/`backup_instance_subfolder.bats`/`promote_credentials.bats` all do this.

---

### `scripts/promote-credentials.sh`

**Responsibility:** The explicit, operator-run half of `SHARED_CREDENTIALS_MODE=seed` — copies this instance's own `.credentials.json` into `SHARED_CREDENTIALS_PATH`, so other `seed`-mode instances pick it up on their next start. Does nothing automatically; nothing else in the project ever invokes it.

**Resolution:** same `scripts/lib/config-path.sh`/`INSTANCE_CONFIG_PATH` resolution as `backup.sh`/`restore.sh`/`status.sh`. Accepts an optional session-name argument to use `.env.<name>` instead of `.env`, matching `session-up.sh`'s convention.

**Refuses to run against a `live`-mode session:** if `CONFIG_DIR/.credentials.json` is a symlink, this is a `live`-mode session (see `docker/entrypoint.sh` development rule 10) — `live` mode already syncs automatically, so there's nothing for this script to do, and it fails with a message saying so rather than silently no-op'ing or, worse, trying to promote a symlink's target path as if it were file content.

**Confirmation:** if `SHARED_CREDENTIALS_PATH` already holds different (non-empty) content, prompts before overwriting — since this write reaches every `seed`-mode instance that hasn't logged in on its own yet, on their next start. `--force` skips the prompt (for scripted/cron use). Identical content needs no prompt.

**Never touches `live`-mode's mechanism, `entrypoint.sh`, or any other instance's own config directory** — purely a host-side file copy between `CONFIG_DIR/.credentials.json` and `SHARED_CREDENTIALS_PATH/.credentials.json`, `chmod 600`'d after writing.
...
5. **SHARED_CREDENTIALS_PATH is a live share, backed by a symlink plus a background poller — not the symlink alone:** `~/.claude/.credentials.json` is symlinked to `SHARED_CREDENTIALS_PATH`'s file at startup (see `docker/entrypoint.sh` development rule 10), but `claude` itself does not write that file in place — a login or token refresh replaces the path with a brand-new regular file (write + `rename()`), which detaches the symlink outright and would otherwise strand the new credentials in the session's own directory forever, invisible to every other session. A background poller (plain bash, started right after the initial link, no new package or daemon outside the container) checks every 5s for exactly that and re-promotes + re-links automatically, logging every time it fires. Net effect: a login/refresh leaves the file outside `SHARED_CREDENTIALS_PATH` for at most a few seconds, not until a restart. What still requires a restart is a session that was already running *before* `SHARED_CREDENTIALS_PATH` was set (or before its shared file existed) — the symlink and its poller are only established once, at startup. If shared login ever stops working, check `dock.log` for `Shared credentials` lines first — they say plainly whether this session is linked, re-linking, or failing to write; if still stuck, delete the credentials file inside `SHARED_CREDENTIALS_PATH` on the host, log in again in one session, then restart the other containers to relink.
...
- [x] `INSTANCE_CONFIG_PATH`: optional, additive override of the `CONFIG_BASE_PATH`/`REMOTE_SESSION_NAME` join, resolved identically by `docker-compose.yml`'s volume interpolation and every host script via the new `scripts/lib/config-path.sh`
- [x] `SHARED_CREDENTIALS_MODE=seed`: opt-in alternative to `SHARED_CREDENTIALS_PATH`'s original live symlink+poller mechanism (still the `live` default, unchanged) — a one-time copy at boot instead of a live link, with explicit promotion via the new `scripts/promote-credentials.sh` instead of automatic, unlocked promotion from every session. Exists specifically to avoid the no-locking multi-writer race `live` mode has (characterized in `tests/entrypoint_shared_credentials_race.bats`) when more than one session promotes near-simultaneously
- [x] `backups/<REMOTE_SESSION_NAME>/`: per-instance backup subfolder (`scripts/backup.sh`'s new default destination once a session name is known), with `scripts/restore.sh` checking both this location and the legacy flat `./backups/` so existing backups stay listable/restorable with no migration step
- [x] `docs/usage-patterns.md`: generic worked examples for Git-versioned, local (no Git), and multi-project instances — none of these require `WORKSPACE_TYPE` or any other new variable, since `GIT_REPO_URL`/`GITHUB_TOKEN_FILE` were already fully independent and optional
- [x] `docs/git-integration.md`: documented the shared-vs-per-instance `GITHUB_TOKEN_FILE` pattern as a first-class option — no code change, since the variable was already just an arbitrary host path
