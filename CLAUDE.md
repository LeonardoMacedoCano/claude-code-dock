...
| Share one Claude Code login across sessions instead of one per `REMOTE_SESSION_NAME` | `SHARED_CREDENTIALS_PATH` | Silent no-op when unset (mounted to `/dev/null`, gated on `-d`); warns (non-fatal) if the host directory isn't writable by the container's user |
