...
10. `SHARED_CREDENTIALS_PATH` link (right after the `~/.claude.json` symlink block, before `HAS_CREDENTIALS` is read): symlinks `~/.claude/.credentials.json` (`SESSION_CREDS`, this session's own, always inside the per-session mount) to a fixed filename (`.credentials.json`) inside `~/.claude-shared-credentials/` (`SHARED_CREDS_FILE`, the dedicated directory mount fed by `SHARED_CREDENTIALS_PATH`, see the `docker-compose.yml` section above) — the same idiom as the `CLAUDE_JSON_LINK`/`CLAUDE_JSON_REAL` symlink just above it, not a copy. Gated on `[ -d "${SHARED_CREDS_DIR}" ]` — true only when the operator actually set `SHARED_CREDENTIALS_PATH` to a real host directory, since the unset `/dev/null` fallback is a character special file, not a directory, so `-d` is false and the whole block is a no-op. When the directory is set but not writable by the container's user (e.g. Docker auto-created it root-owned because the host path didn't exist yet), warns non-fatally and skips the link — this session still logs in on its own, it just doesn't join the shared pool until the operator fixes ownership. Otherwise: if `SESSION_CREDS` is still a *regular* file (a login predating `SHARED_CREDENTIALS_PATH` being set, or a pre-symlink claude-code-dock version), its content — if non-empty — is copied into `SHARED_CREDS_FILE` (overwriting whatever was already there, since a login this session already performed always wins) before the file itself is removed; then `SESSION_CREDS` is `ln -sf`'d to `SHARED_CREDS_FILE` unconditionally. Because it's a symlink and not a copy, every subsequent write `claude` makes to `SESSION_CREDS` — including a first login performed later in that same container's lifetime, which is the common case — lands directly in `SHARED_CREDENTIALS_PATH` with no restart required, and is immediately visible to every other session sharing the same path on its next read.
