...
10. `SHARED_CREDENTIALS_PATH` link (right after the `~/.claude.json` symlink block, before `HAS_CREDENTIALS` is read): symlinks `~/.claude/.credentials.json` (`SESSION_CREDS`, this session's own, always inside the per-session mount) to a fixed filename (`.credentials.json`) inside `~/.claude-shared-credentials/` (`SHARED_CREDS_FILE`, the dedicated directory mount fed by `SHARED_CREDENTIALS_PATH`, see the `docker-compose.yml` section above) — the same idiom as the `CLAUDE_JSON_LINK`/`CLAUDE_JSON_REAL` symlink just above it, not a copy. Gated on `[ -d "${SHARED_CREDS_DIR}" ]` — true only when the operator actually set `SHARED_CREDENTIALS_PATH` to a real host directory, since the unset `/dev/null` fallback is a character special file, not a directory, so `-d` is false and the whole block is a no-op.
    ...
    - **Writable directory:** if `SESSION_CREDS` is still a *regular* file (a login predating `SHARED_CREDENTIALS_PATH` being set, or a pre-symlink claude-code-dock version), its content — if non-empty — is copied into `SHARED_CREDS_FILE` before the file itself is removed, and a login this session already performed always wins (a `log_warn` fires first if `SHARED_CREDS_FILE` already held *different* non-empty content, since this overwrites it — non-fatal, but no longer silent). `SESSION_CREDS` is then `ln -sf`'d to `SHARED_CREDS_FILE` unconditionally. Because it's a symlink and not a copy, every subsequent write `claude` makes to `SESSION_CREDS` — including a first login performed later in that same container's lifetime, which is the common case — lands directly in `SHARED_CREDENTIALS_PATH` with no restart required, and is immediately visible to every other session sharing the same path on its next read. The "session linked ... skips first login" log line only fires when this session loaded a *pre-existing* shared login, never right after this session's own login was the thing that just populated `SHARED_CREDS_FILE`.
...
5. **SHARED_CREDENTIALS_PATH is a live share via symlink, but only takes effect at container start:** `~/.claude/.credentials.json` is symlinked to `SHARED_CREDENTIALS_PATH`'s file at startup (see `docker/entrypoint.sh` development rule 10), so a login performed at any point in that session's lifetime, and a token Claude Code itself rotates later, both write straight through to the shared file with no restart needed. What still requires a restart is a session that was already running *before* `SHARED_CREDENTIALS_PATH` was set (or before its shared file existed) — the symlink itself is only established at startup, so an already-running container keeps using its own local file until it restarts and picks up the link. If shared login ever stops working, delete the credentials file inside `SHARED_CREDENTIALS_PATH` on the host, log in again in one session, then restart the other containers to relink.
