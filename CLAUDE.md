...
refactor(entrypoint): remover SHARED_CREDENTIALS_PATH (nao entregava skip de login de forma confiavel)
```
...
- Do not reintroduce a shared-credentials mechanism (a mount + symlink letting several instances reuse one Claude Code login) without first validating in practice that Claude Code actually treats a credential copied/linked from another instance's `~/.claude` as already-authenticated — a prior version of this project had exactly this (`SHARED_CREDENTIALS_PATH`, live symlink + background poller, later `SHARED_CREDENTIALS_MODE=seed` as a lower-risk alternative) and removed it after real-world testing showed it added a per-instance background process and a no-locking shared-file race without reliably delivering the "skip login" benefit it existed for
...
- [x] Tried, then removed, `SHARED_CREDENTIALS_PATH` (a mount + symlink letting several sessions reuse one Claude Code login, first as a live symlink + background poller, later with a lower-risk `SHARED_CREDENTIALS_MODE=seed` one-time-copy alternative and an explicit `scripts/promote-credentials.sh` promotion step). Real-world testing across real instances showed Claude Code does not reliably treat a credential copied or linked from another instance's `~/.claude` as already-authenticated (neither Remote Control nor plain interactive mode skipped the login screen), so the mechanism added a per-instance background process and a no-locking shared-file race without reliably delivering the "skip login" benefit it existed for. Reverted in full — see git history around this line for the code; each instance now always authenticates independently, once
...
- [x] `docker-publish.yml` now also runs on `pull_request` events targeting `main`, not just `push`/tags/`workflow_dispatch` — previously the `test`/`smoke-test`/lint checks never ran on a PR at all, so a lint or test failure only surfaced after merge, on the resulting push to `main` (a real incident, not hypothetical: a `shellcheck` SC2046 finding in `scripts/restore.sh` shipped in a merge and only failed CI afterward). `build-and-push` stays gated to `github.event_name != 'pull_request'` so images are still only ever built/pushed from something that already landed on `main` (or an explicit tag/`workflow_dispatch` run), never from an open PR branch
