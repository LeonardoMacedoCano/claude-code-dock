...
# gap in SHARED_CREDENTIALS_PATH's live symlink+poller mechanism: promotion
# has no locking. This test asserts CURRENT (unchanged) behavior for the
# default/legacy mode, so it can be used as a before/after comparison once
# the credential-seed mode (SHARED_CREDENTIALS_MODE=seed, a one-time copy
# instead of a live symlink) ships.
#
...
# unrelated PR's CI run flaky for no benefit.
